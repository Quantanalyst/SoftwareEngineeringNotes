...
def solution(A):
    ...
    if (A[0] > 0):
        maximal = A[-1]*A[-2]*A[-3]
    elif (A[0]  < 0) * (A[1]  < 0) & (A[0]*A[1]*A[N-1] > A[-1]*A[-2]*A[-3]):
        maximal = A[0]*A[1]*A[N-1]
    else:
        maximal = A[-1]*A[-2]*A[-3]
    return maximal

# For example, for the input [-5, 5, -5, 4] the solution returned a wrong
# answer (got 100 expected 125).
A = [-5, -6, -4, -7, -10]
solution(A)