...
# of N non-negative integers, specifying the radiuses of the discs, is given.
...
def solution(A):
    N = len(A)
    right_end = [0]*N   # The starting point of discs
    left_end = [0]*N   # The ending point of discs
    for index in range(N):
        right_end[index] = index + A[index]
        left_end[index] = index - A[index]
    right_end.sort()  #--- O(n*logn)
    left_end.sort()   #--- O(n*logn)
    LEI = 0 # left end index (LEI)
    REI = 0 # right end index (REI)
    intersect = 0
    for REI in range(0, N):
        while LEI < N and right_end[REI] >= left_end[LEI]:
            LEI += 1
        intersect += LEI - REI -1
